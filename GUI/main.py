...
def main():
    ...
    st.markdown("""
    **This application uses Natural Language Processing (NLP) and Machine Learning to detect spam emails.**
    ...
    with col2:
        ...
        for title, example in example_emails.items():
            if st.button(title):
                st.session_state['example_email'] = example

        if 'example_email' in st.session_state:
            st.text_area(
                "Selected Example Email:",
                st.session_state['example_email'],
                height=100,
                disabled=True
            )

    # Preprocessing steps information
    ...
    # Footer
    st.markdown("---")
    st.markdown("**💻 Made with ❤️ by Amr Alkhouli**")
